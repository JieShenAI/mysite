...
chart.on("contextmenu", (params) => {
    var ids = ["cityid", "areaid", "id"];
    for (let id in ids) {
        let tmp = params['data'][ids[id]];
        if (typeof (tmp) != 'undefined') {
            curID = tmp;
            break
        }
    }
    curName = params['data']['name'];

    if (!areasChoose.hasOwnProperty(curName)) {
        var h = $(".areasChoose").html();
        $(".areasChoose").html(h + String.format(s, curName));
    }
    areasChoose[curName] = curID;
...
function renderMap(mapTitle, mapJson, customerNum, colorMax = 1500) {
    //地图配置参数，参数按顺序渲染
    option = {
        backgroundColor: "#f2f2f2", //地图画布背景颜色  "#F7EED6"米黄色  "#efefef"灰色
        title: {
            //地图文本
            text: mapTitle,
            subtext: "右键选择地区",
